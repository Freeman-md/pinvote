...
router.post('/', PollValidator.validatePoll, PollController.store)

router.get('/:id/edit', param('id').trim().notEmpty().escape(), PollController.edit)

router.post('/:id/update', param('id').trim().notEmpty().escape(), PollValidator.validatePollUpdate, PollController.update)
